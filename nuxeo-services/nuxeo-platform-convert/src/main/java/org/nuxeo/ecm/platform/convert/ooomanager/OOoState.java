...
public class OOoState {
    ...
    /**
     * Tests whether the server is started. If the servers is in starting state then waits until it is fully started
     * If the server is already started returns true.
     * If it is stopped or stopping returns false.
     * If it is starting then waits using the given timeout until either the server is started or timeout is reached.
     * Returns true if at the end of the method the server is started, false otherwise
     * @param timeout
     * @param unit
     * @return
     * @throws InterruptedException
